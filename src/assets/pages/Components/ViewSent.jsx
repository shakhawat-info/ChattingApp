...
const ViewSent = () => {
  ...
  useEffect(()=>{
    const viewsendRef = ref(db, 'FriendRequests/');
  ...
  let Cancel = (id)=>{
    ...
    remove(ref(db, 'FriendRequests/' + id.key))
  ...
  return (
    <div className="flex flex-col-reverse gap-y-2">
      {requests.map((item , index)=>{
        ...
        <img src={item.val().receiver.user.photoURL} alt="profile" className='lg:w-[80px] w-[60px] lg:h-[80px] h-[60px] rounded-full object-cover  '/>
        <div className="">
          <h5 className='font-aldrich  '>{item.val().receiver.user.displayName}</h5>
